...
| 2 | 11 | изпълнени тестове, не текст; 6/10 | **303/303 ✓** · 26 мутации | **PASS · exit 0** · 9 проверки | **commit** | ✅ |
...
### Състоянието на диска · 2026-09-04 · chunk 1 затворен

| Какво | Число |
|---|---|
| `schemas/` | **6 файла** — 4 обекта, `common`, `observations` |
| `src/` | validator с инварианти · fixture четец с три изхода |
| `scripts/` | gate · мутации · build · generate · drift · record-baseline |
| `scenarios/` | 1 сценарий: `container-oom` |
| `out/core.js` | **213 292 байта**, 4 validator-а, **0 require** |
| `workflows/incident.json` | производен артефакт, сверен с deployment |
| тестове | **204 минават**, 7 файла |
| мутации | **13**, всяка събаря именувания си тест |
| `node scripts/acceptance-gate.mjs` | **PASS · exit 0** · 8 проверки |

**Осемте проверки:** сглобяване без остатъчна зависимост · тестовете минават и не
са нула · типовете · всеки деклариран script сочи реален файл или binary · нищо
с форма на ключ до commit-а · нито един обещан check не е просрочен · всеки от 13
записани дефекта още събаря теста си · генерираният workflow съвпада със записания
deployment.
