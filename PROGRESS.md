...
| 6 | 1 | чистата половина в `merge.ts`; Node чете TypeScript без build | 425/425 ✓ | **PASS · exit 0** · 63 мутации | *(в ход)* | — |
...
| Стъпка | Харчи ли |
|---|---|
| генераторът да вгради петте сглобени инцидента и подканите | не |
| десетте node-а: webhook, assemble, 4×(питай + запиши), conclude | не |
| диференциален тест: TypeScript срещу генерирания JS | не |
| троен преглед | не |
| **петте сценария на живо** | **да — иска думата `харчи`** |
