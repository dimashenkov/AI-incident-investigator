...
| 6 | 21 | **първото повторено измерване · 6 от 6** · $0.0085 | 477/477 ✓ | **PASS · exit 0** · 96 мутации | *(в ход)* | — |
...
`events[0].message`, `pods[0].phase`, `pods[0].containers[0].ready`. Не късмет, а
поведение. Единичното пускане не различава двете, и точно затова прегледът
...
непълен без която и да е. Плюс изричен въпрос, който агентът да си зададе преди
да приключи — „кой ред от таблицата важи за онова, което току-що написах".
